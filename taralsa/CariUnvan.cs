...
namespace taralsa
{
    public partial class CariUnvan : Form
    {
        ...
        CariUnvanCls cari = new CariUnvanCls();
        public void DataGridYenile()
        {
            dgwProducts2.DataSource = cari.TabloyuGetir();
        }
        ...
        {
            cari.YeniSatirEkle(textBox5.Text, textBox4.Text, textBox3.Text, textBox7.Text, textBox1.Text, textBox6.Text);
            DataGridYenile();
        }
        ...
        private void button12_Click(object sender, EventArgs e)
        {
            ...
            {
                string ID = dgwProducts2.Rows[dgwProducts2.CurrentRow.Index].Cells[0].Value.ToString();
                cari.SatirGunceleme(ID, textBox5.Text, textBox4.Text, textBox3.Text, textBox7.Text, textBox1.Text, textBox6.Text);
                DataGridYenile();
            }
            else
            {
        ...
        {
            conn.Open();
            SqlCommand command = new SqlCommand("Select * from CariUnvaniFirma Where CariHesapNo like'%" + textBox5.Text + "%'", conn);
            SqlDataAdapter dataAdapter = new SqlDataAdapter(command);
            DataSet data = new DataSet();
            dataAdapter.Fill(data, "CariUnvaniFirma");
            dgwProducts2.DataSource = data.Tables[0];
            conn.Close();
        }

        private void CariUnvan_Load(object sender, EventArgs e)
        {
